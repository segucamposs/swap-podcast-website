...
export default function SpotifyPlayerCard({ episode }: SpotifyPlayerCardProps) {
  ...
  const titleText = `${episode.title} | ${episode.guest}`;
  ...
  const skip = useCallback((secs: number) => {
    ...
    audioRef.current.currentTime = Math.max(
      0,
      Math.min(audioRef.current.duration || 0, audioRef.current.currentTime + secs)
    );
  }, []);
  ...
  return (
    <>
      {/* Marquee keyframe — injected once per page */}
      <style>{`
        @keyframes swap-marquee {
          0%   { transform: translateX(0); }
          100% { transform: translateX(-50%); }
        }
        .swap-marquee-play {
          animation: swap-marquee 22s linear infinite;
        }
      `}</style>

      <div
        className="relative rounded-2xl overflow-hidden max-w-[480px] mx-auto border"
        style={{ backgroundColor: "#1a1a1a", borderColor: "rgba(255,255,255,0.13)" }}
      >
        <audio
          ref={audioRef}
        ...
        {/* Spotify logo — floats top-right on the card, text scrolls under it */}
        <a
          href={spotifyUrl}
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Abrir en Spotify"
          className="absolute top-4 right-4 z-10 text-white/70 hover:text-white transition-colors duration-150"
        >
        ...
        {/* Top: artwork + info */}
        <div className="flex">
          {/* Artwork — inset with padding + rounded corners */}
          <div className="p-4 pr-3 flex-shrink-0">
            <div className="relative rounded-xl overflow-hidden" style={{ width: 148, height: 148 }}>
              {artwork ? (
                <Image
                  src={artwork}
          ...
          {/* Info */}
          <div className="flex-1 pt-5 pr-4 pb-3 min-w-0 flex flex-col justify-center">
            {/* Scrolling title — text clips at container edge, marquee loops */}
            <div className="overflow-hidden mb-1.5">
              <div className="swap-marquee-play whitespace-nowrap inline-block">
                <span className="text-white font-bold text-[17px]">{titleText}</span>
                <span className="text-white/25 mx-10">•</span>
                <span className="text-white font-bold text-[17px]">{titleText}</span>
                <span className="text-white/25 mx-10">•</span>
              </div>
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 text-white/70 hover:text-white text-sm transition-colors duration-150"
            >
        ...
        <div className="flex items-center gap-3 px-5 pb-5 pt-2">
          ...
          >
            <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              ...
              <text x="12" y="15.5" fontSize="5" textAnchor="middle" fontFamily="sans-serif" fontWeight="bold" fill="currentColor">15</text>
            </svg>
          </button>
          ...
          {/* ··· */}
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-white/35 flex-shrink-0" aria-hidden="true">
